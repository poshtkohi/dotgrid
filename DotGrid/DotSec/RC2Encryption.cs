...
namespace DotGrid.DotSec
{
	...
	public class  RC2Encryption
	{
		...
		public RC2Encryption(byte[] key, byte[] IV)
		{
			if(key == null)
		...
		public RC2Encryption(int KeySize)
		{
		...
		public byte[] encrypt(byte []dataForEncryption)
		{
			...
			RC2CryptoServiceProvider rc2 = new RC2CryptoServiceProvider();
			ICryptoTransform encryptor = rc2.CreateEncryptor(this.key,this.IV);
		...
		public byte[] decrypt(byte []dataForDecryption)
		{
			...
			RC2CryptoServiceProvider rc2 = new RC2CryptoServiceProvider();
			ICryptoTransform decryptor = rc2.CreateDecryptor(this.key,this.IV);
